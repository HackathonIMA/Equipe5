...
/*
  ...
  public function consultarEspera($data = null){
    if($data == null){
  ...
  public function consultarAtraso($data = null){
    if($data == null){
      $dataIni = date('Y/m/d', strtotime("-60 days"));
    }else{
      $dataIni = implode('/', array_reverse(explode('/', $data)));
  ...
  public function contarNovas() {
    $dataIni = date('Y/m/d', strtotime("-7 days"));
    $table = "solicitacoes";
    $fields = "count(*) as quantidade";
    $where = "(data_cadastro>='$dataIni')";
    $retorno = $this -> read($table, $fields, $where, null, null, null);
    return $retorno[0]->quantidade;
  }

  public function contarEspera() {
    $dataIni = date('Y/m/d', strtotime("-60 days"));
    $comando = "select
                avg(DATEDIFF (CURRENT_DATE(), data_cadastro)) Media_dias_Atraso
                from solicitacoes
                ...
                and data_cadastro >= ('".$dataIni."');";
    $retorno = $this -> open($comando);
    return round($retorno[0]->Media_dias_Atraso);
  }

  public function contarAtraso() {
    $dataIni = date('Y/m/d', strtotime("-365 days"));
    $comando = "select
      avg(DATEDIFF (data_conclusao, data_cadastro)) Media_dias_Atraso
      from solicitacoes
      where data_cadastro >= ('".$dataIni."');";
